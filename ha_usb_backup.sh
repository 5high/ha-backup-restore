...
if [ -d "${BACKUP_FOLDER}" ]; then
        ...
        if [ "${INCLUDE_DB}" = true ] ; then
                ...
                sudo zip -9 -q -r ${BACKUP_FILE} . -x"components/*" -x"deps/*" -x".HA_VERSION" -x"home-assistant.log"
		sudo zip -9 -q -r ${BACKUP_FILE1} . -x"components/*" -x"deps/*" -x".HA_VERSION" -x"home-assistant.log"
        else
                log i "Creating backup"
                sudo zip -9 -q -r ${BACKUP_FILE} . -x"components/*" -x"deps/*" -x".HA_VERSION" -x"home-assistant.db" -x"home-assistant_v2.db" -x"home-assistant.log"
		sudo zip -9 -q -r ${BACKUP_FILE1} . -x"components/*" -x"deps/*" -x".HA_VERSION" -x"home-assistant.db" -x"home-assistant_v2.db" -x"home-assistant.log"
        fi
